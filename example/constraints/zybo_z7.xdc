...
## LAN connected to JE
##Pmod Header JE                                                                                                                  
set_property -dict { PACKAGE_PIN V12  IOSTANDARD LVCMOS33 } [get_ports { rmii_txd[0]  }]; #IO_L4P_T0_34    Sch=je[1]
set_property -dict { PACKAGE_PIN W16  IOSTANDARD LVCMOS33 } [get_ports { rmii_rxd[1]  }]; #IO_L18N_T2_34   Sch=je[2]
set_property -dict { PACKAGE_PIN J15  IOSTANDARD LVCMOS33 } [get_ports { rmii_crs_dv  }]; #IO_25_35        Sch=je[3]
set_property -dict { PACKAGE_PIN H15  IOSTANDARD LVCMOS33 } [get_ports { rmii_mdc     }]; #IO_L19P_T3_35   Sch=je[4]
set_property -dict { PACKAGE_PIN V13  IOSTANDARD LVCMOS33 } [get_ports { rmii_tx_en   }]; #IO_L3N_T0_DQS_34 Sch=je[7]
set_property -dict { PACKAGE_PIN U17  IOSTANDARD LVCMOS33 } [get_ports { rmii_rxd[0]  }]; #IO_L9N_T1_DQS_34 Sch=je[8]
set_property -dict { PACKAGE_PIN T17  IOSTANDARD LVCMOS33 } [get_ports { rmii_ref_clk }]; #IO_L20P_T3_34   Sch=je[9]  clock input from LAN8720
set_property -dict { PACKAGE_PIN Y17  IOSTANDARD LVCMOS33 } [get_ports { rmii_mdio    }]; #IO_L7N_T1_34    Sch=je[10]

set_property -dict { PACKAGE_PIN T14   IOSTANDARD LVCMOS33} [get_ports { rmii_txd[1] }]; #IO_L5P_T0_34 Sch=jd_p[1]                  

set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets rmii_ref_clk_IBUF]

## Ref Clock is created by the LAN Module - we need to consume it
create_clock -add -name clk_50_rmii -period 20.00 -waveform {0 10} [get_ports { rmii_ref_clk }];  

## 
# reate_generated_clock -name clk_mdc \
#    -source [get_pins block_design_i/clk_wiz_0/inst/clkout1_buf/O] \
#    -divide_by 126 \
#    [get_ports rmii_mdc]
# 
# et_output_delay -clock clk_mdc -max 10.0 [get_ports rmii_mdio]
# et_output_delay -clock clk_mdc -min -10.0 [get_ports rmii_mdio]
# 
# et_false_path -from [get_ports rmii_mdio] \
#               -to   [get_pins -hierarchical -filter {NAME =~ *r_reg\[mdio_meta\]*/D}]
# 
